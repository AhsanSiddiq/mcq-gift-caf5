...
async function importEcoMCQs() {
  console.log(`Starting Economic MCQ batch 2 import for ${SUBJECT_ID}...`);

  const dataPath = path.join(__dirname, 'data', 'prc3-eco-batch2.json');
  const rawData = fs.readFileSync(dataPath, 'utf-8');
  const mcqs = JSON.parse(rawData);

  console.log(`Loaded ${mcqs.length} questions from prc3-eco-batch2.json`);
