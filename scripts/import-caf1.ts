...
const subjectId = 'caf-1';

async function parseAndSeed(filenames: string[]) {
  ...
  console.log(`Successfully seeded ${totalSuccess} questions in total.`);
}

async function run() {
  await parseAndSeed(['caf1-p1.json', 'caf1-p2.json', 'caf1-p3.json']);
}
