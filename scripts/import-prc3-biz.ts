...
async function importMCQs() {
  console.log(`Starting MCQ import for ${SUBJECT_ID}...`);

  const dataPath = path.join(__dirname, 'data', 'prc3-biz.json');
  const rawData = fs.readFileSync(dataPath, 'utf-8');
  const mcqs = JSON.parse(rawData);

  console.log(`Loaded ${mcqs.length} questions from prc3-biz.json`);
  ...
  for (const item of mcqs) {
    ...
    if (originalCorrect.match(/^[A-D]\)/)) {
        ...
        const matchingOption = item.options.find(opt => opt.includes(originalCorrect) || opt === originalCorrect);
        if (matchingOption) {
    ...
    // 5. Insert options
    const optionsToInsert = item.options.map((optStr) => {
