...
async function parseAndSeed(filename: string) {
  ...
  for (const q of questions) {
    ...
    if (existing) {
      console.log(`Question caf6-${q.id} already exists, skipping.`);
      successCount++;
      continue;
    }
...
async function run() {
  await parseAndSeed('caf6-p1.json');
  await parseAndSeed('caf6-p2.json');
  await parseAndSeed('caf6-p3.json');
}
