...
async function importEcoMCQs() {
  console.log(`Starting Economic MCQ batch 4 import for ${SUBJECT_ID}...`);

  const dataPath = path.join(__dirname, 'data', 'prc3-eco-batch4.json');
  const rawData = fs.readFileSync(dataPath, 'utf-8');
  const mcqs = JSON.parse(rawData);

  console.log(`Loaded ${mcqs.length} questions from prc3-eco-batch4.json`);
  ...
  for (const item of mcqs) {
    ...
    // Mapping Economics chapters to C + 8 (e.g., Chapter 1 becomes Chapter 9)
