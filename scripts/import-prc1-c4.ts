...
// Regex to strip any AI citation markers like [1], [1, 2], [1, 2, 3] etc.
const citationRegex = /\s*\[\s*\d+(?:\s*,\s*\d+)*\s*\]/g;

async function importChapter4() {
  console.log('Starting PRC-1 Chapter 4 (Accruals and Prepayments) Import...');

  const dataDir = path.join(__dirname, 'data');
  const files = ['prc1-c4-1.json', 'prc1-c4-2.json', 'prc1-c4-3.json', 'prc1-c4-4.json'];
  ...
  for (const q of allQuestions) {
    ...
    // Strip any residual AI citation markers for safety
    const cleanExplanation = q.explanation
      ? q.explanation.replace(citationRegex, '').trim()
      : '';
    ...
    if (optError) {
      ...
      count++;
      if (count % 10 === 0) {
        console.log(`  Progress: ${count}/${allQuestions.length} questions seeded...`);
      }
    }
  }

  console.log(`\nCompleted! Successfully seeded ${count} / ${allQuestions.length} questions for PRC-1 Chapter 4.`);
}
