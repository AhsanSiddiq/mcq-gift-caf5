...
async function importEcoMCQs() {
  console.log(`Starting Economic MCQ batch 5 import for ${SUBJECT_ID}...`);

  const dataPath = path.join(__dirname, 'data', 'prc3-batch5-eco.json');
  const rawData = fs.readFileSync(dataPath, 'utf-8');
  const mcqs = JSON.parse(rawData);

  console.log(`Loaded ${mcqs.length} questions from prc3-batch5-eco.json`);
