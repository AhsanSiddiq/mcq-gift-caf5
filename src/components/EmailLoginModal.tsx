...
export default function EmailLoginModal({ isOpen, onClose, onSuccess }: Props) {
  ...
  useEffect(() => {
    if (!isOpen) {
      setTimeout(() => { setStep("email"); setOtp(["","","","","",""]); setError(""); setResendCooldown(0); }, 300);
    }
  }, [isOpen]);
          ...
          <>
            <div className="flex items-center gap-3 mb-6">
              ...
              <div>
                ...
                <p className="text-xs" style={{ color: "var(--text-3)" }}>Sync across all devices · No password needed</p>
              </div>
            </div>
            <form onSubmit={handleSendOTP} className="flex flex-col gap-3">
              <div className="relative">
                ...
                <input
                  ...
                  style={{ background: "var(--bg-3)", border: "1px solid var(--border)", color: "var(--text-1)", fontFamily: "var(--font-inter), sans-serif" }}
        ...
        {step === "otp" && (
          <>
            ...
            <p className="text-sm mb-3 leading-relaxed" style={{ color: "var(--text-2)" }}>
              We sent a 6-digit code. Valid for <strong>10 minutes</strong>.
            </p>

            {/* ⚠️ Spam warning — RIGHT IN THE POPUP where users need it */}
            <div className="rounded-xl px-3 py-2.5 mb-4 flex items-start gap-2" style={{ background: "rgba(251,191,36,0.08)", border: "1px solid rgba(251,191,36,0.25)" }}>
              <span className="text-sm" style={{ lineHeight: 1.4 }}>⚠️</span>
              <p className="text-xs leading-relaxed" style={{ color: "#ca8a04" }}>
                <strong>Can&apos;t find it?</strong> Check your <strong>Spam</strong> or <strong>Junk</strong> folder and mark it &quot;Not Spam&quot; so future codes land in your inbox.
              </p>
            </div>
            ...
            <div className="flex items-center justify-between mt-3">
              ...
              <button
                ...
                style={{ color: resendCooldown > 0 ? "var(--text-3)" : "var(--green)", background: "none", border: "none", opacity: resendCooldown > 0 ? 0.5 : 1 }}
              >
        ...
        {step === "success" && (
          <div className="flex flex-col items-center text-center py-4">
            ...
            <p className="text-sm" style={{ color: "var(--text-2)" }}>
              Saved to <strong style={{ color: "var(--text-1)" }}>{email}</strong>. Access your progress on any device.
            </p>
          </div>
        )}
      </div>
